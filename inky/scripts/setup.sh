...
# Step 2: Install inky project with hardware dependencies
echo "Step 2: Installing Inky project with hardware dependencies..." >&2
cd "$INKY_ROOT"
...
if [ "$INSTALL_SUCCESS" = false ]; then
    ...
    if [ "$UV_AVAILABLE" = true ]; then
        echo "Installing dependencies with uv (including hardware support)..." >&2
        if [ -f "$UV" ]; then
            if "$UV" pip install -e ".[dev,hardware]" 2>&1; then
                INSTALL_SUCCESS=true
                echo "✓ Dependencies installed with uv (hardware support included)." >&2
            fi
        elif "$UV" pip install -e ".[dev,hardware]" 2>&1; then
            INSTALL_SUCCESS=true
            echo "✓ Dependencies installed with uv (hardware support included)." >&2
        fi
    fi
...
# Fall back to pip
if [ "$INSTALL_SUCCESS" = false ]; then
    echo "Using pip to install dependencies (including hardware support)..." >&2
    if "$PIP" install -e ".[dev,hardware]" 2>&1; then
        INSTALL_SUCCESS=true
        echo "✓ Dependencies installed with pip (hardware support included)." >&2
    else
        echo "Warning: Failed to install with dev dependencies, trying core dependencies..." >&2
        if "$PIP" install -e ".[hardware]" 2>&1; then
            echo "✓ Core dependencies installed with hardware support. Dev dependencies may be missing." >&2
            echo "  You can install them later with: $PIP install -e \".[dev]\"" >&2
            INSTALL_SUCCESS=true
        else
            echo "Error: Failed to install package with hardware support" >&2
...
if ! _check_package_installed; then
    ...
    echo "  You can try manually: $PIP install -e \".[dev,hardware]\"" >&2
    exit 1
fi
